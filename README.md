## [홈페이지 원문 보기](https://pm2.keymetrics.io/docs/usage/pm2-doc-single-page/)
...
PM2는 [데몬프로세스](https://blogger.pe.kr/770) 관리자이며, 당신의 온라인 어플리케이션 관리를 돕습니다. PM2를 시작하는것은 간단합니다. 
NPM을 통해 설치할 수 있는 간단하고 직관적인 CLI 를 제공합니다.

## Installation

The latest PM2 version is installable with NPM or Yarn:

최신 PM2 버전은 NPM 또는 Yarn 을 통해 설치가능 합니다. 
...
[Node.js 그리고 NPM 설치하러가기. NVM](https://yoember.com/nodejs/the-best-way-to-install-node-js/)

## Start an app

The simplest way to start, daemonize and monitor your application is by using this command line:

응용 프로그램을 시작, 데몬 및 모니터링하는 가장 간단한 방법은 다음 명령 줄을 사용하는 것입니다.
...
# 파일이 변경될때마다 관찰 및 재시작 하기
--watch

# Set memory threshold for app reload
# 앱 재로드를위한 메모리 임계 값 설정
...
보이는바와 같이 PM2에는 어플리케이션을 관리하는데 사용할 수 있는 많은 옵션이 있다. 사용사례에 따라 더 발견해 나갈 수 있을것입니다.

## Managing processes

Managing application state is simple here are the commands:

어플리케이션 상태를 관리하는것은, 여기 명령어로 간단히 가능합니다
...
이 애플리케이션을 시작 했으므로 pm2.io를 사용하여 상태, 로그, 메트릭을 확인하고 온라인 대시 보드를 이용 할 수 있습니다.

### List managed applications

List the status of all application managed by PM2:

PM2에 관리되고 있는 모든 어플리케이션 상태를 나열합니다.
...
Node.js 애플리케이션의 경우 PM2에는 생성 된 각 프로세스간에 모든 HTTP(s) / Websocket / TCP / UDP 연결을 공유하는 자동로드 밸런서가 포함되어 있습니다.

To start an application in Cluster mode:

클러스터 모드에서 응용 프로그램을 시작하려면
...
또한 Ecosystem 이라는 설정 파일을 만들어 여러 응용 프로그램을 관리 할 수 있습니다. Ecosystem 파일을 생성하려면
...
그러면 ecosystem.config.js 파일이 생성됩니다.
...
서버 부팅 / 재부팅시 관리하는 프로세스로 PM2를 다시 시작하는 것은 위험할 수 있습니다. 이를 해결하려면 다음 명령을 실행하여 활성 시작 스크립트를 생성하십시오.
...
그리고 자동 리스폰에 대한 프로세스 목록을 고정 시키려면 :
...
현재 디렉토리 + 모든 하위 폴더의 파일 변경 사항에 따라 앱을보고 다시 시작하며 node_modules 폴더의 변경 사항을 무시합니다 --ignore-watch = "node_modules".
...
우리는 그것을 간단하게 만들었습니다. 릴리스간에 주요 변경 사항이 없으며 절차는 간단합니다
...
알아야 할 몇 가지 명령은 다음과 같습니다. 샘플 응용 프로그램이나 개발 컴퓨터의 현재 웹 응용 프로그램으로 시도해보십시오.
